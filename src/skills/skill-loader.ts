...
export const BUILTIN_SKILLS: SkillDefinition[] = [
  ...
  },
  {
    name: 'ultra-workflow',
    description:
      'Ultra Workflow 超工作流——把 council（多席评审）、team（波次执行）、galaxy（星河并行）三个编排机制串成一条全链路：先审再干、按波落地、并行攻坚。输入 /ultra-workflow <任务> 或描述含"超工作流""全链路编排""把这三个结合"时激活。',
    triggers: [
      /ultra-workflow/i,
      /\/ultra/i,
      /超工作流/,
    ...
    builtIn: true,
    body: [
      '# Ultra Workflow — 三合一全链路编排',
      '',
      '你进入 Ultra Workflow 模式。目标：单会话内从**新手大白话需求 → 工业级可交付代码**全链路落地，',
      '流程：**需求澄清+环境基线 → council 评审 → team 波次执行 → galaxy 并行攻坚 → 交付门禁**。',
      '三个底层工具（council/team/galaxy）均已存在，本工作流负责编排协议与质量门禁。',
      '',
      '## 人话进度协议（贯穿全程）',
      '',
      '每个阶段开始/结束时，用大白话向用户报告：**刚完成了什么 / 接下来要做什么 / 你不需要懂细节**。',
      '任何机制性回退（council 复议 / team 重派 / galaxy 维度失败）必须附带一段人话版错误解释：',
      '报错是什么、为什么发生、下一步是什么。新手不需要看懂编排，但必须随时知道"现在到哪了、卡在哪"。',
      '',
      '## 总览',
      '',
      '```',
      '大白话需求 ──► [0. 澄清+基线] ──► [1. council 评审] ──通过──► [2. team 波次] ──► [3. galaxy 攻坚] ──► [4. 交付门禁] ──► 交付',
      '                 │ 歧义/缺环境            │ 驳回/未派发                  │ 波次失败        │ 维度失败',
      '                 └── 问清/脚手架 ────────┴── 修订后复议 ─────────────────┴── 回 council ───┘',
      '```',
      '',
      '## 阶段 0：需求澄清 + 环境基线（新手友好）',
      '',
      '用户可能用大白话描述需求（"帮我做个能记录东西的小工具"）。先**至多一轮**澄清转成结构化输入：',
      '- 目标：一句话复述用户要什么（不含执行细节）',
      '- 非目标：明确不做什么（避免范围蔓延）',
      '- 验收标准：用户可观察的完成标志。示例模板（给新手照抄改）："当我【动作】，【系统】会【可观察结果】"',
      '  如："当我点保存，数据写进文件，关掉再开还在"',
      '- 歧义点：真正阻塞的才问，一次问完；能推断的不问',
      '',
      '**环境基线探测**（澄清后必做）：探测当前项目是否存在、有无 package.json / typecheck 命令 / git。',
      '向用户输出环境状态：绿=就绪 / 黄=缺 X（我帮你装，装依赖前先确认）。',
      '若项目不存在（从零到一）：进入**脚手架子流程**——git init、package.json、tsconfig、测试框架、',
      'typecheck 命令，逐个建立后再进阶段 1。脚手架是写操作，触碰安全闸门（创建文件/装依赖前确认）。',
      '',
      '**draftItems 构造**（阶段 0→1 交接）：把目标/非目标/验收标准映射为 council 的 draftItems',
      '（每条含 id/title/detail/files），让评审针对具体条目而非一个光秃秃的 objective 字符串。',
      '',
      '## 阶段 1：council 评审（先审再干）',
      '',
      '调用 `council_convene({ objective, draftItems, rounds })` 召集星域多席评审：',
      '- 高风险（架构/跨模块/安全）rounds: 2；低风险 rounds: 1',
      '',
      '**门禁守卫（防静默跳过）**：council 返回后先检查是否真的派发了席位——',
      '若结果 isError，或返回文本含"已禁用（COUNCIL=0）""未派发任何席位"→ 视为**评审未执行**，',
      '禁止进入阶段 2；如实告知用户评审被禁用，问是否降级为自评或修复配置。',
      '驳回/冲突（blocking 冲突未化解）同样视为未通过——修订 draftItems 后重新评审，不得绕过。',
      '',
      '**评审通过后的衔接（二选一，明确路径）**：',
      '- 路径 A：`council_convene({ ..., autoExecute: true })`——审查通过后自动派发执行',
      '- 路径 B：取 council 编译产出的 UnifiedPlan JSON 作为 `team_orchestrate({ planJson })` 输入',
      '- 禁止：把 council 的审计 Markdown（# 议事会计划/席位贡献/裁决记录）当 planMarkdown 直接传给 team——',
      '  它不含任务分片结构，team 解析会失败',
      '',
      '## 阶段 2：team 波次执行（按波落地）',
      '',
      '调用 `team_orchestrate({ objective, planJson })` 按波次派发：',
      '- 分片：把计划切成文件不重叠的正交分片，每个分片端到端（实现+测试）',
      '- 依赖排序：两个分片必须改同一文件时设 dependsOn 排序，校验器拦重叠',
      '- wave-gate：每波完成验证后再进下一波；不自动提交（NOT auto-commit）',
      '- 波次失败 → 回阶段 1 复议缩小范围，不硬闯',
      '',
      '## 阶段 3：galaxy 并行攻坚/审查（并行攻坚）',
      '',
      '处理执行中出现的独立子问题或全局审查。**保留两阶段确认**：',
      '- 第一次调用 `galaxy({ objective, dimensions, confirm: false })` 展示维度方案',
      '  （拆哪些维度、激活哪些星域、派多少 worker）',
      '- 用户确认后，第二次调用 `galaxy({ objective, dimensions, confirm: true })` 才执行',
      '- 可写维度必须文件范围不重叠；多星域仅只读视角（不共享实时上下文）',
      '- 维度失败 → 回阶段 2 重新派发对应分片',
      '',
      '## 阶段 4：工业级交付门禁（按任务分级）',
      '',
      '交付前逐项核销（缺一项不算完成）。**门禁按任务复杂度分级，默认从严、显式降级**：',
      '- 小工具/一次性脚本（如"能记录东西的小工具"）：最小可运行 + 运行演示通过 + 三项交付报告',
      '- 工业级/长期维护项目：完整五项——',
      '  1. typecheck：项目自有 typecheck 命令 exit 0（测试通过不能代偿类型门禁）',
      '  2. 消费方核对：改过导出符号签名后 grep 全部消费方，接口两侧同步',
      '  3. 语义回归：真实输入覆盖降级路径，不取信宽松 mock',
      '  4. 测试门禁：RED→GREEN 有据，报告数字来自本轮工具输出（禁止记忆报数）',
      '  5. 交付报告：做了什么 / 遗留什么 / 设计偏差（如有）三项覆盖',
      '未运行 = 说"未验证"；命令启动未终止 = 同样未验证。全绿后 deliver_task 提交。',
      '',
      '## 兜底与收尾',
      '',
      '- 任一阶段连续失败 2 次 → 缩小任务范围重试，不做无谓的重试循环',
      '- 交付：所有阶段通过后按交付纪律提交（deliver_task 门禁）',
      '- 报告：输出全流程审计轨迹（基线状态、评审裁决、波次调度、维度聚合、门禁核销），并附人话版总结',
    ].join('\n'),
  },
]
