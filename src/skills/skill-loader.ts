...
export const BUILTIN_SKILLS: SkillDefinition[] = [
  ...
  },
  {
    ...
    body: [
      ...
      '',
      '你进入 Ultra Workflow 模式。目标：大型任务按「**council 评审 → team 波次执行 → galaxy 并行攻坚**」三阶段自动编排，',
      '实现"先审再干、按波落地、并行攻坚"。三个底层工具均已存在，本工作流只负责编排协议。',
      '',
      '## 总览',
      '',
      '```',
      '大型任务 ──► [1. council 评审] ──通过──► [2. team 波次执行] ──► [3. galaxy 并行攻坚/审查] ──► 聚合交付',
      '                 │ 驳回                         │ 波次失败                    │ 维度失败',
      '                 └────── 返回方案修订 ──────────┴──── 回到 council 复议 ───────┘',
      '```',
      '',
