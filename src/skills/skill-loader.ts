...
export const BUILTIN_SKILLS: SkillDefinition[] = [
  ...
  },
  {
    ...
    body: [
      ...
      '流程：**需求澄清 → council 评审 → team 波次执行 → galaxy 并行攻坚 → 交付门禁**。',
      '三个底层工具（council/team/galaxy）均已存在，本工作流负责编排协议与质量门禁。',
      '',
      '## 总览',
      '',
      '```',
      '大白话需求 ──► [0. 需求澄清] ──► [1. council 评审] ──通过──► [2. team 波次执行] ──► [3. galaxy 并行攻坚] ──► [4. 交付门禁] ──► 交付',
      '                 │ 歧义                       │ 驳回                        │ 波次失败            │ 维度失败',
      '                 └── 一次问清 ────────────────┴── 返回方案修订 ─────────────┴── 回 council 复议 ───┘',
      '```',
      '',
      '## 阶段 0：需求澄清（新手友好）',
      '',
      '用户可能用大白话描述需求（"帮我做个能记录东西的小工具""把那个流程自动化一下"）。',
      '进入执行前，先用**至多一轮**澄清把模糊需求转成结构化输入：',
      '- 目标：一句话复述用户要什么（不含执行细节）',
      '- 非目标：明确不做什么（避免范围蔓延）',
      '- 验收标准：用户可观察的完成标志（"当我点保存，数据落盘且刷新后还在"）',
      '- 歧义点：真正阻塞的才问，一次问完；能推断的不问（先完成能确定的部分）',
      '确认后带着结构化输入进入阶段 1。新手小白不需要懂编排细节——你负责翻译。',
      '',
      '## 阶段 1：council 评审（先审再干）',
      '',
      '调用 `council_convene({ objective, draftItems, rounds: 2 })` 召集星域多席评审任务方案：',
      '- 评审通过（确定性裁决）→ 进入阶段 2，把裁决后的计划作为 team 的输入',
      '- 评审驳回 → 按席位意见修订方案后重新评审，不要跳过评审直接执行',
      '- 高风险（架构/跨模块/安全）任务必须完整走评审；低风险可单轮咨询（rounds: 1）',
      '',
      '## 阶段 2：team 波次执行（按波落地）',
      '',
      '调用 `team_orchestrate({ objective, planMarkdown 或 planJson })` 按波次派发：',
      ...
      '',
      '调用 `galaxy({ objective, dimensions, confirm: true })` 处理执行中出现的独立子问题或全局审查：',
      '- 独立子问题（前后端联调、多视角交叉验证）→ 拆 2-5 个维度并行',
      '- 全局审查 → autoReview 默认追加瑶光审查，跨维度冲突在聚合报告单独列出',
      '- 可写维度必须文件范围不重叠；多星域仅只读视角（不共享实时上下文）',
      '- 维度失败 → 回阶段 2 重新派发对应分片',
      '',
      '## 阶段 4：工业级交付门禁',
      '',
      '所有阶段通过后，交付前逐项核销（缺一项不算完成）：',
